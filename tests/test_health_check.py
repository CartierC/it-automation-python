...
def test_check_cpu_returns_metric_result(mock_cpu):
    result = check_cpu(threshold=85.0)
...
def test_check_memory_returns_metric_result(mock_mem):
    mock_mem.return_value = MagicMock(percent=TEST_MEM_MOCK_VALUE, total=16 * 1024 ** 3)
...
def test_check_memory_status_alert(mock_mem):
    mock_mem.return_value = MagicMock(percent=88.0, total=16 * 1024 ** 3)
    result = check_memory(threshold=80.0)
    assert result.status == "ALERT"
...
def test_check_disk_returns_metric_result(mock_disk):
    ...
    result = check_disk(threshold=90.0)
    assert isinstance(result, MetricResult)
